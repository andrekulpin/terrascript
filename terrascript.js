...
client.on('run', function(){
	...
	client.isGenerator && keepGeneratorStatus(client);
	!client.isGenerator && becomeGenerator(client);

})

client.on('ready', function(){

	require('async_methods')(client);
	client.run();
...
module.exports = function(client){
	ClientEnhancer.call(client);	
}

function ClientEnhancer(){
	this.sendMessages = function(){
		async.until(
			...
			function(done){
				setTimeout(function(){
					this.lpush('messages', 1, function(err, res){
						done();
					})
				})
			},
	...
	this.getMessage = function(){
		async.until(
			...
			function(done){
				this.rpop('messages', function(err, res){
					done();
				})
			},
			function(err){}
	...
	this.becomeGenerator = function(){
		async.until(
			...
			function(done){
				setTimeout(function(){
					this.setnx('generator_id', process.pid, function(err, res){
						if(res){
	...
	this.keepGeneratorStatus = function(){
		async.until(
			function(){
				return !this.isGenerator;
			},
